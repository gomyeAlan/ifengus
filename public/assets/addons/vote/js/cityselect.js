...
(function ($, window) {
     ...
     * 内部函数
     */
    functionality.throttle = function (func, wait, options) {
    ...
    functionality.search = functionality.throttle(function (event) {
        var self = this,
            $target = $(event.target),
            configure = this.options,
        ...
        if (keyCode !== 13 && keyCode !== 38 && keyCode !== 40) {
            ...
            $.each(self.contain, function (key, value) {
                var lowercases = value.pinyin.toLocaleLowerCase();
            ...
            resultHtml = functionality.montage.apply(self, [results]) || '<li class="not-have">没有找到<strong>' + inputVal + '</strong>相关城市</li>';
        ...
        if (val) {
            ...
            for (var i = 0; i < cityArray.length; i++) {
                ...
                $.each(self.newCityData, function (key, value) {
                    ...
                    if ((isNaN(cityArray[i]) && cityArray[i] === value.name) || (!isNaN(cityArray[i]) && cityArray[i] == value.id)) {
