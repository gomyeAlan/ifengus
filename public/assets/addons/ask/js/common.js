$(function () {
    ...
    if ($question.size() > 0 && Config.loadmode === 'infinite') {
        var maxLoadPage = $question.data("max-load-page") ? parseInt($question.data("max-load-page")) : 1;
        var loadList = function () {
            var page = parseInt($question.data("page"));
            page = page ? page : 1;
            page++;
            // 这里可以取消注释可以变更URL链接和标题
            // if (history.pushState) {
            //
            //     var title = document.title;
            //     title = title.replace(/\s+-\s+第(\d+)页/, " - ");
            //     title = title.replace(/\s+\-\s+/, " - 第" + page + "页");
            //     var url = location.href;
            //     var pushurl = url.match(/(\?|&)page=(\d+)?/) ? url.replace(/(\?|&)page=(\d+)?/, '$1page=' + page) : (url + (url.indexOf("?") > -1 ? "&page=" + page : "?page=" + page));
            //     try {
            ...
            $.ajax({
                ...
                beforeSend: function (xhr) {
                    $question.after($('<div class="loadmore"><span class="loadmore-tips">加载中...</span></div>').fadeIn()).data("loading", true);
                },
                success: function (data) {
                    $(".loadmore").remove();
                    $(".pager", $question).remove();
                    if (data) {
                        var $data = $(data);
                        $question.append($data);
                        $question.removeData("loading");
                        $question.data("page", page);
                    } else {
                        $question.append('<div class="loadmore loadmore-line"><span class="loadmore-tips">已全部加载</span></div>');
                    }
                }, error: function () {
        ...
        $(window).scroll(function () {
            if (!$question.data("loading") && (!$question.data("page") || parseInt($question.data("page")) <= maxLoadPage) && !$(".loadmore-nodata", $question).size() && $question.data("more")) {
                if ($(window).scrollTop() - $question.height() > $question.offset().top - $(window).height()) {
                    loadList();
                }
            }
        });
