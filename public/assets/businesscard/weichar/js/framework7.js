...
(function () {
    ...
    window.Framework7 = function (params) {
        ...
        app.router._load = function (view, options) {
            ...
            if (app.params.pushState && !options.reloadPrevious && view.main)  {
                ...
                if (pushState) {
                    if (!isDynamicPage && !pageName) {
                        history[method]({url: url, viewIndex: app.views.indexOf(view)}, '', pushStateRoot + app.params.pushStateSeparator + url+"&to=detail");
                    }
            ...
            if (animatePages) {
                ...
                if (app.params.material && app.params.materialPageLoadDelay) {
                	...
                	/*if(oldPage[0].baseURI!=newPage[0].baseURI )*/
                    app.router.animatePages(oldPage, newPage, 'to-left', view);
                }
        ...
        ************   Modals   ************
        ...
        app.calert = function (text, title, callbackOk) {
            if (typeof title === 'function') {
                callbackOk = arguments[1];
                title = undefined;
            }
            return app.modal({
                text: text || '',
                title: typeof title === 'undefined' ? app.params.modalTitle : title,
                buttons: [ {text: app.params.modalButtonClose, bold: true, onClick: callbackOk} ]
            });
        };
